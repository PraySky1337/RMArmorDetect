...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Enhanced pose preview with cls/color/size info")
    parser.add_argument(
        "--model",
        type=Path,
        default=Path("runs/pose/train/weights/best.pt"),
...
def postprocess(preds, orig_shape, preprocess_info, conf_thres, iou_thres, model):
    """Parse raw model output to get detections with cls, color, size, conf, keypoints."""
    if isinstance(preds, (tuple, list)):
        preds = preds[0]

    pred_data = preds[0]  # (nc_color + nc_size + nc_num + nk, na)

    nc_color = model.model[-1].nc_color
    nc_size  = model.model[-1].nc_size
    nc_num   = model.model[-1].nc_num
    nkpt, ndim = model.model[-1].kpt_shape
    nk = nkpt * ndim
    ...
    color_conf, color_idx = color_scores.max(dim=0)
    size_conf, size_idx   = size_scores.max(dim=0)
    num_conf, num_idx     = num_scores.max(dim=0)

    # Combined confidence
    combined_conf = color_conf * size_conf * num_conf
    ...
    valid_color = color_idx[valid_mask]
    valid_size  = size_idx[valid_mask]
    valid_num   = num_idx[valid_mask]
    valid_kpts  = kpt_data[:, valid_mask]
    ...
    final_conf  = valid_conf[keep].cpu().numpy()
    final_color = valid_color[keep].cpu().numpy()
    final_size  = valid_size[keep].cpu().numpy()
    final_num   = valid_num[keep].cpu().numpy()
...
def draw_detections(img, detections):
    ...
    for det in detections:
        ...
        color_name = COLOR_NAMES[color_idx] if color_idx < len(COLOR_NAMES) else "?"
        size_name  = SIZE_NAMES[size_idx]  if size_idx  < len(SIZE_NAMES)  else "?"
        cls_name   = CLASS_NAMES[num_idx]  if num_idx   < len(CLASS_NAMES) else "?"
        bgr_color = COLORS.get(color_name, (0, 255, 0))
        ...
        # Draw label: color + size + number
        label = f"{color_name}{size_name}{cls_name} {conf:.2f}"
        x, y = kpts[0]
        cv2.putText(img, label, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, bgr_color, 2, cv2.LINE_AA)

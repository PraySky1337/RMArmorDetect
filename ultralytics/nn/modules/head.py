...
class Pose(nn.Module):
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor | tuple:
        ...
        if self.dynamic or self.shape != shape:
            ...
            # anchors: (na, 2), strides: (na, 1)
            self.anchors = anchors.transpose(0, 1)  # (2, na)
            self.strides = strides.squeeze(-1)  # (na,) instead of (1, na)
            self.shape = shape
    ...
    def _init_end2end(self, ch: tuple, c3_num: int, c3_color: int, c3_size: int, c4: int) -> None:
        """Initialize end-to-end detection components if enabled."""
        if hasattr(self, 'end2end') and self.end2end:
            self.one2one_cv3 = nn.ModuleList(
                nn.Sequential(DWConv(x, x, 3), Conv(x, c3_num, 1), Conv(c3_num, c3_num, 3), Conv(c3_num, self.nc_num, 1))
                for x in ch
            )
            self.one2one_cv_color = nn.ModuleList(
                nn.Sequential(DWConv(x, x, 3), Conv(x, c3_color, 1), Conv(c3_color, c3_color, 3), Conv(c3_color, self.nc_color, 1))
                for x in ch
            )
            self.one2one_cv_size = nn.ModuleList(
                nn.Sequential(DWConv(x, x, 3), Conv(x, c3_size, 1), Conv(c3_size, c3_size, 3), Conv(c3_size, self.nc_size, 1))
                for x in ch
            )
    ...
    def kpts_decode(self, kpts: torch.Tensor) -> torch.Tensor:
        ...
        if self.export:
            ...
            # Ensure proper broadcasting: y shape is (bs, nk, na),
            # anchors[0] is (na,), strides is (na,)
            ...
            # Clamp keypoints to image bounds to avoid negative values or out-of-range predictions
